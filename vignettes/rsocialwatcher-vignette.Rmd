---
title: "rsocialwatcher: Usage Examples"
#author: "Robert Marty"
#date: "`r Sys.Date()`"
...
* [Setup](#setup)
* [Querying Parameter IDs](#query_param)
  * [User Type IDs](#param_users)
  * [Location Key](#param_locs)
  * [Location Key with Coordinates/Geometries](#param_locs)
* [Get coordinates/geometries for location keys](#coord_geom)
* [Querying Data](#query_data)
  * [Different Location Types](#query_data_loc)
    * [Radius around lat/lon](#query_data_loc_latlon)
    * [Location IDs - Country, State, District, City, Neighborhood, etc.](#query_data_loc_keys)
      * [Country Groups](#query_data_loc_keys_country_groups)
      * [Country](#query_data_loc_keys_country)
      * [Region](#query_data_loc_keys_region)
      * [Large Geographic Area](#query_data_loc_keys_lga)
      * [Medium Geographic Area](#query_data_loc_keys_mga)
      * [Small Geographic Area](#query_data_loc_keys_sga)
      * [City](#query_data_loc_keys_city)
      * [Subcity](#query_data_loc_keys_subcity)
      * [Neighborhood](#query_data_loc_keys_neighborhood)
      * [Zip Code](#query_data_loc_keys_zipcode)
      * [Geo Market (U.S. Designated Market Areas)](#query_data_loc_keys_geo_market)
      * [Electoral Districts](#query_data_loc_keys_electoral)
  * [Querying data using parameter IDs](#query_data_ids)
    * [Using one parameter ID per parameter type](#query_data_ids_options)
    * [And and OR Categories](#query_data_ids_andor)
      * [Within parameter types](#query_data_ids_andor_within)
      * [Across parameter types: Flexible Targetting](#query_data_ids_andor_flex)
    * [Excluding Categories](#query_data_ids_exclude)
  * [Multiple Queries](#query_data_ids_multquery)
    * [Simple Examples](#query_data_ids_multquery_simple)
    * [And/Or Conditions](#query_data_ids_multquery_andor)
...
```{r get_sugg_radius, include=TRUE, warning=FALSE, message=FALSE}
...
* Enter multiple parameter IDs within __vectors c()__ to specify OR conditions
* Enter multiple parameter IDs within __lists list()__ to specify AND conditions
...
#### Across parameter types: Flexible Targeting <a name="query_data_ids_andor_flex"></a>
...
__Example 4: OR and AND Conditions__
The below shows a more complicated example, which grabs all users that meet (EITHER interests:1 or behaviors:3) AND (EITHER behaviors:2 or behaviors:4). Everything within `flex_param[[1]]` are grouped within OR conditions, but different indices (`flex_param[[1]]` and `flex_param[[2]]`) are separated by AND conditions.
```
...
### Excluding Categories <a name="query_data_ids_exclude"></a>
...
__Example 1: Simple example,__ making one query to determining the number of users with an interest in `1`, and a separate query to determine the number of users with an interest in `2`.
```{r map_param_ex1, include=TRUE, warning=FALSE, message=FALSE}
map_param(1, 2)
```

__Example 2: Note that below will only make one query,__ querying users with an interest in 1 OR 2. There is only one item: `c(1,2)` -- which the function interprets using an `OR` condition.
...
__Example 3: To map over a vector,__ we can use the `map_param_vec()` function---which creates a separate query for each item in the vector.
