...
#### Flexible Targetting: Or and And

Flexible tagetting, using the `flex_target` parameter, allows for more complicated parameter specification. The input for `flex_target` follows the following logic:
...
int_music_id <- interests_df %>% filter(name == "Music (entertainment & media)") %>% pull(id)
beh_comm_id  <- behaviors_df %>% filter(name == "Owns: Galaxy S6") %>% pull(id)
...
## Separate query for number of users in US and Mexico and commuters and frequenty travelers
beh_comm_id      <- behaviors_df %>% filter(name == "Commuters") %>% pull(id)
beh_freqtrvl_id  <- behaviors_df %>% filter(name == "Frequent Travelers") %>% pull(id)
